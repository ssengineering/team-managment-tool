...
function printOpenNotes(){
	global $db;
	try {
		$notificationQuery = $db->prepare("SELECT * FROM executiveNotification WHERE status ='1' ORDER BY startDate,startTime ASC");
	...
	while($cur = $notificationQuery->fetch(PDO::FETCH_ASSOC)){
		echo '<input type="radio" name="id" id="id" value="'.$cur['ID'].'" />Started on: '.date("l, M j, Y",strtotime($cur['startDate'])).', for ticket '.$cur['ticketNum'].', Subject: '.$cur['subject'].'  <a href="deleteNoteClosed.php?id='.$cur['ID'].'">Delete</a><br />';
	}
}
?>
